...
/**
 * Azure Sequencer, version 2.0.
 */
public class NGSoundSeqConverter {
	...
	public static class AzSeq {
		...
		public AzSeq(Sequence seq, int trackNo, int channelNo) {
			instrumentNames.add("GrandPno");

			Track track = seq.getTracks()[trackNo];
			...
			float currentTempoScale = 500000f / defaultTickScale / 1000f * 0.693333f;
			...
			for (int i = 0; i < track.size(); i++) {
				...
				if (cmd == 0x80 + channelNo) {
					...
					int key = mesg[1] & 0xFF;
					float nowTime = evt.getTick() * currentTempoScale;
					// System.out.println("NoteON " + nowTime + " acttick " +
					// evt.getTick());
